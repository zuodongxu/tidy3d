...
from ..data.terminal import AntennaParametersData, PortDataArray, TerminalPortDataArray
...
class TerminalComponentModeler(AbstractComponentModeler):
    ...
    def compute_power_wave_amplitudes_at_each_port(
        ...
        """Helper to compute the incident and reflected power wave amplitudes at each port for a given
    ...
    def compute_power_wave_amplitudes(
        ...
        """Helper to compute the incident and reflected power wave amplitudes at a lumped port for a given
        simulation result. The computed amplitudes have not been normalized.
        """
    ...
    def compute_power_delivered_by_port(
        ...
        """Helper to compute the total power delivered to the network by a lumped port for a given
        simulation result. Units are in Watts.
        """
    ...
    def get_radiation_monitor_by_name(self, monitor_name: str) -> DirectivityMonitor:
        """Find and returns the associated :class:`.DirectivityMonitor` monitor.
    ...
    @staticmethod
    def _combine_directivity_data(
        mon_data_1: DirectivityData, mon_data_2: DirectivityData
    ) -> DirectivityData:
        """Form the superposition of two :class:.`DirectivityData`. Flux is recomputed by
        integrating the projected fields over a sphere.
        """
        fields_dataset = mon_data_1.fields_spherical + mon_data_2.fields_spherical
        combined_data = DirectivityData.from_spherical_field_dataset(
            mon_data_1.monitor, fields_dataset
        )
        return combined_data

    def get_antenna_parameters_data(
        self, port_amplitudes: dict[str, complex] = None, monitor_name: str = None
    ) -> AntennaParametersData:
        ...
        for port, amplitude in port_dict.items():
            ...
            if amplitude is None:
                directivity_data = sim_data_port[rad_mon.name]
                scale_factor = 1.0
            else:
                directivity_data = self._monitor_data_at_port_amplitude(
            ...
            if combined_directivity_data is None:
                combined_directivity_data = directivity_data
            else:
                combined_directivity_data = self._combine_directivity_data(
                    combined_directivity_data, directivity_data
                )
            a_sum += a
            b_sum += b
        ...
        return AntennaParametersData.from_directivity_data(
