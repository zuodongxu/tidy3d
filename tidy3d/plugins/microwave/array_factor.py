...
def make_antenna_array(
    ...
    extend_dims = [dim for dim, size in enumerate(array_size) if size > 1]
        ...
        for dim in extend_dims:
            ...
            if extends_beyond_min or extends_beyond_max:
                ...
                if (isinstance(obj, Structure) 
                    ...
                    and extends_beyond_min and extends_beyond_max):
                    continue
    ...
    for dim, spacing in zip(extend_dims, spacings):
    ...
    antenna_array_size = (np.array(array_size) - 1) * np.array(spacings) + antenna_size
    ...
    for i in range(array_size[0]):
        for j in range(array_size[1]):
            for k in range(array_size[2]):
                ...
                translation_vector = -sim_size / 2 - sim_bounds[0] + np.array([i, j, k]) * np.array(spacings)
                ...
                for source in simulation.sources:
                    ...
                    phase_diff = i * phase_shifts[0] + j * phase_shifts[1] + k * phase_shifts[2]
        ...
        if isinstance(monitor, AbstractFieldProjectionMonitor):
            ...
            for dim in extend_dims:
                if mnt_size[dim] != np.inf:
                    ...
                    mnt_bounds[0][dim] = mnt_bounds[0][dim] - sim_bounds[0][dim] - sim_size[dim] / 2

                    # shift max bounds to new location
                    mnt_bounds[1][dim] = mnt_bounds[0][dim] + monitor.size[dim] + (array_size[dim] - 1) * spacings[dim]
...
def rectangular_antenna_array_factor(
        array_size: Tuple[PositiveInt, PositiveInt, PositiveInt], 
        spacings: Tuple[NonNegativeFloat, NonNegativeFloat, NonNegativeFloat],
        phase_shifts: Tuple[float, float, float],
    ...
    spacings : Tuple[NonNegativeFloat, NonNegativeFloat, NonNegativeFloat]
        Spacing between elements in the x, y, and z directions (in micrometers).
    phase_shifts : Tuple[float, float, float]
        Phase shift between elements in the x, y, and z directions (in radians).
    theta : Union[float, ArrayLike]
    ...
    psi_x = k * spacings[0] * np.sin(theta_array) * np.cos(phi_array) - phase_shifts[0]
    psi_y = k * spacings[1] * np.sin(theta_array) * np.sin(phi_array) - phase_shifts[1]
    psi_z = k * spacings[2] * np.cos(theta_array) - phase_shifts[2]

    # Calculate the array factor in the x, y, and z directions
    af_x = np.sum(np.exp(-1j * np.arange(array_size[0])[:, None, None] * psi_x[np.newaxis, :]), axis=0)
    af_y = np.sum(np.exp(-1j * np.arange(array_size[1])[:, None, None] * psi_y[np.newaxis, :]), axis=0)
    af_z = np.sum(np.exp(-1j * np.arange(array_size[2])[:, None, None] * psi_z[np.newaxis, :]), axis=0)
...
def monitor_data_from_array_factor(
    ...
    spacings : Tuple[NonNegativeFloat, NonNegativeFloat, NonNegativeFloat]
        The spacing between elements in the x, y, and z directions (in micrometers).
    phase_shifts : Tuple[float, float, float]
        The phase shift between elements in the x, y, and z directions (in radians).
    new_monitor : AbstractFieldProjectionMonitor = None
    ...
    af = rectangular_antenna_array_factor(array_size, spacings, phase_shifts, theta.ravel(), phi.ravel(), freqs)
    ...
    if isinstance(monitor_data, DirectivityData):
        ...
        s = 0.5 * np.real(monitor_data.Etheta * np.conj(monitor_data.Hphi) - monitor_data.Ephi * np.conj(monitor_data.Htheta))
...
def simulation_data_from_array_factor(
    ...
    sim_array = make_antenna_array(antenna_data.simulation, array_size, spacings, phase_shifts)
    ...
    for mnt_data in antenna_data.data:
        ...
        if mnt_name in mnt_dict:
            array_mnt_data = monitor_data_from_array_factor(
