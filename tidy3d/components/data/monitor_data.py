...
class MonitorData(AbstractMonitorData, ABC):
    ...
    def scale_fields_by_freq_array(
        ...
        Only has effect on :class:`.MonitorData` in the frequency domain, which has been produced by a source
        in a :class:`.Simulation`. For example, has no effect on :class:`.ModeSolverData`.
        """
...
class FieldProjectionAngleData(AbstractFieldProjectionData):
    ...
    def get_phi_slice(
        ...
        """Get a planar slice of the :class:`.FieldProjectionAngleDataArray` defined
        by a given angle `phi`. The returned array will have a length of one along `phi`, and
        `theta` will now have double the length and cover a full 2π radians.
        """
...
class DirectivityData(FieldProjectionAngleData):
    ...
    def from_spherical_field_dataset(
        ...
        """Creates a :class:.`DirectivityData` from a xarray dataset.
        Unlike :class:.`DirectivityData` created by a :class:.`DirectivityMonitor`, flux
        will be computed by integrating the power density over a spherical surface. Thus,
        the suppled ``field_dataset`` must sufficiently sample the entire spherical surface.
        """
    ...
    def calc_radiation_efficiency(self, power_in: FreqDataArray) -> FreqDataArray:
        """Given the power supplied to a radiating element, calculates the radiation efficiency."""
