...
class LayerRefinementSpec(Box):
    ...
    def _is_inplane_unbounded(self) -> bool:
        """Whether the layer is unbounded in any of inplane dimensions."""
...
class GridSpec(Tidy3dBaseModel):
    ...
    **Lectures:**
    ...
    def all_snapping_points(
        self,
        structures: List[Structure],
        cached_internal_snapping_points: List[CoordinateOptional] = None,
    ) -> List[CoordinateOptional]:
        ...
        structures : List[Structure]
            List of physical structures.
        cached_internal_snapping_points : List[CoordinateOptional]
            If not `None`, skip recomputing internal snapping points.
        ...
        if cached_internal_snapping_points is None:
            return self.internal_snapping_points(structures) + list(self.snapping_points)
        return cached_internal_snapping_points + list(self.snapping_points)
    ...
    def all_override_structures(
        ...
        wavelength: pd.PositiveFloat,
        sim_size: Tuple[float, 3],
        cached_internal_override_structures: List[MeshOverrideStructure] = None,
    ) -> List[StructureType]:
        ...
        wavelength : pd.PositiveFloat
            Wavelength to use for minimal step size in vaccum.
        cached_internal_override_structures : List[MeshOverrideStructure]
            If not `None`, skip recomputing internal override structures.
        ...
        if cached_internal_override_structures is None:
            return (
                self.internal_override_structures(structures, wavelength, sim_size)
                + self.external_override_structures
            )

        return cached_internal_override_structures + self.external_override_structures
    ...
    def make_grid(
        ...
        all_structures = list(structures) + self.all_override_structures(
            ...
            wavelength,
            sim_size,
            cached_internal_override_structures,
        )
        ...
        for idim, (dim, grid_1d) in enumerate(zip("xyz", grids_1d)):
            coords_dict[dim] = grid_1d.make_coords(
                ...
                snapping_points=self.all_snapping_points(
                    structures, cached_internal_snapping_points
                ),
            )
